...
class CeThon_GlobalCollection(BASE):
    __tablename__ = "cethon_globalcollection"
    ...
    def __repr__(self):
        return "<Cethon Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, CeThon_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


CeThon_GlobalCollection.__table__.create(checkfirst=True)
...
def add_to_collectionlist(keywoard, contents):
    with ZE_GLOBALCOLLECTION:
        keyword_items = CeThon_GlobalCollection(keywoard, tuple(contents))
...
def del_keyword_collectionlist(keywoard):
    with ZE_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(CeThon_GlobalCollection.keywoard)
            .filter(CeThon_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(
            CeThon_GlobalCollection.keywoard).distinct().all()
...
@CeThon.on(events.NewMessage(outgoing=True, pattern=r"\.تحديث"))
async def _(event):
